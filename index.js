...
class YoutubeBulkToMP3 {
  async download(filePath) {
    ...
    for (let i = 0; i < links.length; i++) {
      ...
      const data = await exec(`/usr/local/bin/youtube-dl -x ${links[i]}`);
  ...
  async _convertToMp3(pos, totalLinks, outputArray) {
    ...
    if (fileName) {
      ...
      await exec(
        `/usr/local/bin/ffmpeg -i "${fileName}" -b:a 192K -vn "${noExt}.mp3"`
      );
  ...
  _getFileName(outputArray) {
    try {
      let found = outputArray.find(
        (out) => out.indexOf("[ffmpeg] Destination:") !== -1
      );
      if (!found) {
        found = outputArray.find(
          (out) => out.indexOf("[download] Destination:") !== -1
        );
      }
      if (found) {
